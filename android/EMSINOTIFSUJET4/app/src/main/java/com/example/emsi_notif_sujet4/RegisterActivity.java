    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        private void RegisterPersonne()
        {
            ...
            else
            {
                ...
                perso.setUsername(usernamePerso); perso.setPassword(password); perso.setFullname(fullname); perso.setEmail(email); perso.setNiveau(niveau);
                perso.setTypePersonne(t_Personne); perso.setFiliere(f_Personne); perso.setGroupe(g_Personne); perso.setDepartement(d_Personne); perso.setAge(age);

                Call<Personne> call=ipersonneService.addPersonne(perso);
                call.enqueue(new Callback<Personne>() {
                    @Override
                    public void onResponse(Call<Personne> call, Response<Personne> response) {
                        if(response.isSuccessful())
                        {
                            Toast.makeText(RegisterActivity.this, "Data inserted In Rest ", Toast.LENGTH_SHORT).show();
                        }
